...
from trackpull.aggregate import AggregateConfig, aggregate, validate_nan_policy
...
class TestExport:
    def test_points_group_created(self, mock_source, hdf5_store):
    ...
    def test_run_ids_correct(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store)
        ids = hdf5_store.read_field(POINTS_GROUP, "run_id")
        assert set(ids) == {"r1", "r2", "r3"}

    def test_scalar_values_correct(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store)
        widths = hdf5_store.read_field(POINTS_GROUP, "model.width")
        assert set(widths) == {64.0, 128.0}

    def test_history_shape(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store, history=True)
        mat = hdf5_store.read_field(POINTS_GROUP, "energy_step")
        assert mat.shape == (3, 3)  # 3 runs × 3 steps

    def test_history_values(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store, history=True)
        mat = hdf5_store.read_field(POINTS_GROUP, "energy_step")
        # r1 last step: -1.2
        r1_idx = list(hdf5_store.read_field(POINTS_GROUP, "run_id")).index("r1")
    ...
    def test_transforms_applied(self, hdf5_store):
        ...
        export(cfg, _Src(), hdf5_store)
        col = hdf5_store.read_field(POINTS_GROUP, "hidden_dims")
...
class TestAggregate:
    def test_statistics_group_created(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store)
        _run_aggregate(hdf5_store)
        fields = hdf5_store.list_fields(STATISTICS_GROUP)
        assert "mean_energy" in fields
        assert "std_energy" in fields
        assert "mean_variance" in fields

    def test_group_by_values(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store)
        _run_aggregate(hdf5_store)
        widths = hdf5_store.read_field(STATISTICS_GROUP, "model.width")
        assert set(widths) == {64.0, 128.0}

    def test_mean_correct(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store)
        _run_aggregate(hdf5_store)
        widths = hdf5_store.read_field(STATISTICS_GROUP, "model.width")
        means = hdf5_store.read_field(STATISTICS_GROUP, "mean_energy")
        w64_idx = int(np.where(widths == 64.0)[0][0])
        # r1=-1.2, r2=-1.5 → mean=-1.35
        assert means[w64_idx] == pytest.approx(-1.35)

    def test_single_group_no_group_by(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store)
        cfg = AggregateConfig(
            group_by=[],
            aggregations={"energy": ["mean"]},
        )
        aggregate(cfg, hdf5_store)
        mean_energy = hdf5_store.read_field(STATISTICS_GROUP, "mean_energy")
        # mean of -1.2, -1.5, -2.0 — single group, shape (1,)
        assert float(mean_energy.flat[0]) == pytest.approx((-1.2 - 1.5 - 2.0) / 3)

    def test_history_aggregate_shape(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store, history=True)
        cfg = AggregateConfig(
            group_by=["model.width"],
        ...
        aggregate(cfg, hdf5_store)
        mean_curve = hdf5_store.read_field(STATISTICS_GROUP, "mean_energy_step")
        # 2 groups × 3 timesteps
        assert mean_curve.shape == (2, 3)

    def test_unknown_group_by_raises(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store)
        cfg = AggregateConfig(
    ...
    def test_unknown_agg_function_raises(self, mock_source, hdf5_store):
        _run_export(mock_source, hdf5_store)
        cfg = AggregateConfig(
            group_by=["model.width"],
...
class TestNanPolicy:
    ...
    def test_warn_policy_logs_warning(self, hdf5_store, caplog):
        src = self._make_source_with_nan()
        export(ExportConfig(["width"], ["energy"]), src, hdf5_store)
        cfg = AggregateConfig(
            group_by=["width"],
            aggregations={"energy": ["mean"]},
            nan_policy="warn",
    ...
    def test_ignore_policy_no_warning(self, hdf5_store, caplog):
        src = self._make_source_with_nan()
        export(ExportConfig(["width"], ["energy"]), src, hdf5_store)
        cfg = AggregateConfig(
            group_by=["width"],
    ...
    def test_raise_policy_raises(self, hdf5_store):
        src = self._make_source_with_nan()
        export(ExportConfig(["width"], ["energy"]), src, hdf5_store)
        cfg = AggregateConfig(
            group_by=["width"],
